...
import com.shoppingcart.CartActivity;
...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final int i) {
        ...
        viewHolder.removeToCart.setVisibility(View.VISIBLE);
        viewHolder.addToCart.setText("Update");
        ...
        viewHolder.addToCart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    ((CartActivity) context).cartDetail();
                    //data.get(i).setQty(1);
                    Toast.makeText(context, "Product Update To Cart", Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        ...
        viewHolder.removeToCart.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                try {
                    if (db.deleteRecord(data.get(i).getCart_item_id(), DBHelper.CARTITEM_TABLE) == 1) {
                        data.remove(i);
                        notifyItemRemoved(i);
                        notifyItemRangeChanged(i, data.size());
                        ((CartActivity) context).cartDetail();
                    } else {
                        Toast.makeText(context, "Database Internal error", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView addToCart;
        @BindView(R.id.remove_to_cart)
        TextView removeToCart;
